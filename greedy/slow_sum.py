...
def getTotalTime(arr):
    ...
    sorted_arr = list(np.sort(arr))[::-1]

    accumulator = sorted_arr.pop(0)
    max_sums = []

    for _ in range(0, len(sorted_arr)):
        next_greatest = sorted_arr.pop(0)
        accumulator += next_greatest
        max_sums.append(accumulator)
