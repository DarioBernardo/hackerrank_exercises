...
should return the integer proportional to its weight in the w array. For example, for w = [1, 3],
...
class Solution:
    ...
    def __init__(self, w: List[int]):
        single_probs = [x / sum(w) for x in w]
        self.probs = [single_probs[0]]
        for i in range(1, len(single_probs)):
            self.probs.append(single_probs[i] + self.probs[-1])
