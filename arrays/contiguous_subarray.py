...
def logic(arr: List[int]) -> List[int]:
    solution = []

    for pos, elem in enumerate(arr):
        cursor = pos - 1
        windows_number = 0
        while cursor >= 0 and arr[cursor] < elem:
            cursor -= 1
            windows_number += 1

        solution.append(windows_number)
    return solution
...
def count_subarrays(arr):
    ...
    # what about same values? i.e. [3 ,3 ,3 ,3 ,]
    ...
    s_0_list = logic(arr)
    s_1_list = logic(arr[::-1])[::-1]
    solution = []
    for s0, s1 in zip(s_0_list, s_1_list):
        solution.append(s0 + s1 + 1)
