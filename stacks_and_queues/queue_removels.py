...
def findPositions(arr, x):
    ...
    for iteration in range(0, x):
        pops_number = min(x, len(arr))
        max_value = -1
        max_value_index = -1
        for pop_iteration in range(0, pops_number):
            elem = arr.pop(0)
            index_elem = indexes.pop(0)
            if elem > max_value:
                max_value = elem
                max_value_index = index_elem

            arr_stack.append(elem)
            index_stack.append(index_elem)

        result.append(max_value_index)

        for stacks_index in range(0, len(arr_stack)):
            stack_value = arr_stack.pop(0)
            stack_value_index = index_stack.pop(0)

            if stack_value_index != max_value_index:
                arr.append(max(stack_value - 1, 0))
                indexes.append(stack_value_index)
